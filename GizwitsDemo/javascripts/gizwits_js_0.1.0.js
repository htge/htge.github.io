...
GizwitsJS.prototype.editGroupName = function(params) {
    this._editGroupName(params.group_id, params.group_name);
};

GizwitsJS.prototype.editGroupDevices = function(params) {
    var dids = new Array();
    var count = 0;
    if (params.devices) {
        for (var i=0; i<params.devices.length; i++) {
            var device = params.devices[i];
            dids[count++] = device.did;
        }
    }
    this._editGroupDevices(params.group_id, dids);
};
...
GizwitsJS.prototype.deleteGroupDevices = function(params) {
    ...
    this._deleteGroupDevices(params.group_id, dids);
};
...
GizwitsJS.prototype._editGroupName = function (gid, name) {
    var gizJS = this;
    var url = "https://{0}/app/group/{1}".format(gizJS._apiHost, gid);
    var data = JSON.stringify({
        group_name: name
...
GizwitsJS.prototype._editGroupDevices = function (gid, dids) {
    var gizJS = this;
...
GizwitsJS.prototype._updateGroupDevices = function (gid) {
    ...
    $.ajax(url, {
        ...
        .done(function(result) {
            ...
            gizJS.onUpdateGroupDeviceList({devices: groupDevicesList}, null);
        })
        .fail(function(evt) {
            gizJS.onUpdateGroupDeviceList(null, {
                group_id: gid,
